export var __ParticleCount: number = 2000;
const ParticleCountSlider = document.getElementById('slider3') as HTMLInputElement | null;
if (!ParticleCountSlider) {
  throw new Error("Unable to get value of ParticleCountSlider");
}
ParticleCountSlider.addEventListener("input", () => {
    __ParticleCount = Number(ParticleCountSlider?.value);
}); 


export var __abstractness: number = 0.8; 
const AbstractnessSlider = document.getElementById('slider4') as HTMLInputElement | null;
if (!AbstractnessSlider) {
throw new Error("Unable to get value of ParticleCountSlider");
}
AbstractnessSlider.addEventListener("input", () => {
    __abstractness = Number(AbstractnessSlider?.value);
});

export var __speed: number = 1; 
const SpeedSlider = document.getElementById('slider5') as HTMLInputElement | null;
if (!SpeedSlider) {
  throw new Error("Unable to get value of SpeedSlider");
}
SpeedSlider.addEventListener("input", () => {
    __speed = Number(SpeedSlider?.value);
});

export var __colorValue: number = 0; 
const colorValueSlider = document.getElementById('slider6') as HTMLInputElement | null;
if (!colorValueSlider) {
  throw new Error("Unable to get value of colorValueSlider");
}
colorValueSlider.addEventListener("input", () => {
...
export const ColorPalettes = [
                        ['#FF00FF', '#00FFFF', '#FFA500', '#FFD700', '#FF1493'], // Neon
                        ['#8B4513', '#B8860B', '#CD853F', '#D2691E', '#A0522D'], // Vintage
                        ['#C7B198', '#9C7647', '#825E3A', '#533D32', '#3C2F2F'],  // Old Painter Style Pictures
                        ['#FFD1DC', '#FFB6C1', '#FFA07A', '#ADD8E6', '#B0E0E6'], // Pastel
                        ['#0077be', '#0087b3', '#3ea6cf', '#5b9bd5', '#4c4c4c'], // Ocean
                        ['#F5B041', '#F5AB35', '#F4D03F', '#F9E79F', '#FAD7A0'], // Sunset
                        ['#8B4513', '#D2691E', '#8F9779', '#556B2F', '#CD853F'], // Earthy Tones
                        ['#800080', '#4B0082', '#7B3F00', '#008080', '#FF00FF'], // Jewel Tones
                        ['#FF0000', '#00FF00', '#0000FF', '#FFFF00', '#FFA500'], // Bright and Bold
                        ['#FFFFFF', '#D3D3D3', '#A9A9A9', '#808080', '#696969'], // Monochrome
